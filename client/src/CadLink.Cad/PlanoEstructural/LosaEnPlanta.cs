...
public static class LosaEnPlanta
{
    ...
    /// <summary>
    /// ¿Es una <b>losa de escalera</b> —o de rampa, o un descanso—? Lo dicen la etiqueta, las
    /// notas o la sección.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Se pidió tal cual: «nada de losa de escalera en planos». Y es lo correcto, porque una
    /// escalera <b>no es un tablero de losa</b>: no se arma con parrilla, no lleva bastones por
    /// claro y no se cota como un paño. En el plano de planta va su hueco y su detalle aparte, y
    /// no un tablero armado que nadie va a construir.
    /// </para>
    /// <para>
    /// Se reconoce por el <b>texto</b> y no por la geometría, por el mismo motivo que el
    /// voladizo: la escalera en el modelo es un shell inclinado como cualquier otro, y el que
    /// sabe que es una escalera es el ingeniero, que lo escribe en la propiedad. Deducirlo de la
    /// inclinación fallaría con la rampa a 6% y con el descanso, que es horizontal.
    /// </para>
    /// <para>
    /// Se mira la <b>etiqueta primero</b>, igual que en <see cref="DiceLosacero"/>: la propiedad
    /// puede llamarse «SLAB1» y quien lo dice de verdad son las notas o la etiqueta.
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Devuelve la palabra y no solo un sí o un no porque el descarte se <b>avisa</b>: el aviso
    /// dice qué losa se quitó y por qué palabra. Sin eso, una losa que desaparece del plano
    /// porque su sección se llama «ESCALONADA» es un misterio de media tarde.
    /// </remarks>
    ...
    /// <summary>
    /// <b>Quita de la planta</b> las losas de escalera. Devuelve las palabras que las delataron.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Se quitan de la <b>lista</b>, en un solo sitio y antes de dibujar nada, en lugar de
    /// preguntar «¿es escalera?» en cada paso del dibujo. No es comodidad: la losa se lee en
    /// muchos sitios —el contorno, el achurado, la parrilla, el rótulo, la unión de tableros, la
    /// lista de voladizos y hasta el recuadro del título de la planta—, y un filtro puesto solo
    /// en el que dibuja el contorno dejaría la escalera fuera del papel pero <b>dentro</b> de
    /// todo lo demás: agrupada con el tablero de al lado, cambiándole el armado, y estirando el
    /// recuadro del título hasta el hueco de la escalera.
    /// </para>
    /// <para>
    /// Solo se tocan las <b>losas</b>. Una columna o una trabe cuya nota diga ESCALERA es
    /// estructura de verdad —el apoyo de la propia escalera— y tiene que salir en el plano; por
    /// eso el parámetro se llama <c>IGNORAR_LOSA_ESCALERA</c> y no «ignorar escaleras».
    /// </para>
    /// </remarks>
    public static List<string> Descartar(List<ElementoPlanta> elementos, string palabras)
    {
        var quitadas = new List<string>();
        ...
        for (var i = elementos.Count - 1; i >= 0; i--)
        {
            ...
            var porque = PalabraEscalera(el.Etiqueta, el.Notas, el.Seccion, palabras);
